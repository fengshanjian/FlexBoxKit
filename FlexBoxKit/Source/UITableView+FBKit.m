...
@implementation UITableView (FBKit)
...
- (CGFloat)fb_heightForIndexPath:(NSIndexPath *)indexPath {
    NSMutableArray <NSMutableArray *>*cacheLayout = CACHE_LAYOUT;
    FBKViewLayoutCache *viewLayoutCache = nil;
...
- (UIView *)fb_cacheContentView:(NSIndexPath *)indexPath {
    ...
    if (!cellContentView || [cellContentView isEqual:[NSNull null]]) {
        ...
        FBKCellBlock cellBlock = objc_getAssociatedObject(self, @selector(fb_setCellContnetViewBlockForIndexPath:));
        ...
        if (cellBlock) {
            ...
            if (viewLayoutCache) {
                [cellContentView.layout applyLayoutCache:viewLayoutCache];
            } else {
                [cellContentView.layout applyLayoutPreservingOrigin:NO withSize:CGSizeMake(self.fb_constrainedWidth?:self.frame.size.width, YGUndefined)];
                if (self.fb_cacheLayout) {
                    [cacheLayout[indexPath.section] insertObject:[cellContentView.layout layoutCache] atIndex:indexPath.row];
                }
            }
